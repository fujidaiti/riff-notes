...
describe("computeLabelPlacements", () => {
  it("labels a lone note with its pitch class name", () => {
    const out = computeLabelPlacements(part([note("n", 0, 60)]), 16, L22, 22);
  ...
  it("suppresses dense same-pitch notes within 6 steps (proximity rule)", () => {
    ...
    const out = computeLabelPlacements(part(notes), 64, L22, 22);
  ...
  it("re-anchors a recurring pattern about once per bar", () => {
    ...
    const out = computeLabelPlacements(part(notes), 48, L22, 22);
  ...
  it("ignores notes outside the part range", () => {
    const out = computeLabelPlacements(part([note("lo", 0, 40), note("ok", 0, 64)]), 16, L22, 22);
